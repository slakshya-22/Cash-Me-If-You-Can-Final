...
import {
  ...
  type GenerateTriviaQuestionsInput,
} from "@/ai/flows/generate-trivia-questions-flow";
...
export function useGameState() {
  ...
  const [audiencePollResults, setAudiencePollResults] =
    useState<AudiencePollResults | null>(null);
      ...
      if (answer.isCorrect) {
        ...
        if (currentQuestionIndex === questions.length - 1) {
          ...
          toast({
            ...
            description: `Absolutely brilliant! You've answered all ${
              questions.length
            } questions correctly! Your final score: ${newScore.toLocaleString()} points.`,
            variant: "default",
            duration: 10000,
          });
          return;
        ...
        toast({
          ...
          }". You walk away with ${score.toLocaleString()} points.`,
          variant: "destructive",
          duration: 4000,
        });
      }
  ...
  const loadQuestions = useCallback(
    async (mode?: string, category?: string) => {
      ...
      try {
        const input: GenerateTriviaQuestionsInput = {
          ...
          requestIdentifier: `session_${Date.now()}_${Math.random()
            .toString(36)
            .substring(7)}`,
          variationHint: Math.random().toString(36).slice(2, 12),
        };
        console.log(
          "[useGameState] Generating questions with input:",
        ...
        const questionsWithPoints = aiResult.questions.map((q, index) => ({
          ...q,
          points:
            POINTS_LADDER[index] || POINTS_LADDER[POINTS_LADDER.length - 1],
        }));
        ...
        console.error(
          "[useGameState] Failed to generate/load trivia questions:",
          e
        );
        let displayErrorMessage =
          "Could not generate new trivia questions. Please try again.";
        if (e && e.message) {
          if (
            e.message.includes(
            ...
            e.message.includes(
              "A server error occurred during AI question generation"
            )
          ) {
            displayErrorMessage =
              "The AI failed to generate questions due to a server error. Please check server logs for details and try again.";
          } else if (
            e.message.includes("AI returned malformed data") ||
            e.message.includes(
              "AI prompt execution failed to produce a structured output"
            ) ||
            e.message.includes("AI Flow Error")
          ) {
            ...
            displayErrorMessage =
              "API Key for AI services is missing or invalid. Please check server configuration.";
          } else {
  ...
  useEffect(() => {
    if (currentQuestion) {
      ...
      console.warn(
        "[useGameState] Game is 'playing' but currentQuestion is null. This might indicate an issue with question indexing or an empty question set post-load."
      );
      setDisplayedAnswers([]);
  ...
  const useFiftyFifty = useCallback(() => {
    ...
    const incorrectAnswersFromOriginal = originalAnswers.filter(
      (a) => !a.isCorrect
    );
    ...
    if (correctAnswer && incorrectAnswersFromOriginal.length > 0) {
      ...
      const newDisplayedAnswers = shuffleArray([
        correctAnswer,
        oneRandomIncorrect,
      ]);
  ...
  const useAudiencePoll = useCallback(() => {
    ...
    const correctAnswerInFullSet = currentQuestion.answers.find(
      (a) => a.isCorrect
    );

    if (!correctAnswerInFullSet) {
      toast({
        title: "Audience Poll Error",
        description: "Could not determine correct answer for poll.",
        variant: "destructive",
      });
      return;
    }
    ...
    const isCorrectAnswerDisplayed = optionsForPoll.some(
    ...
    const otherOptions = optionsForPoll.filter(
      (opt) => opt.text !== correctAnswerText
    );
    ...
    otherOptions.forEach((option, index) => {
      if (index === otherOptions.length - 1) {
        ...
        const maxForThisOption = Math.max(
          0,
          totalPercentage - (otherOptions.length - 1 - index)
        );
        const randomPercentage = Math.floor(
          Math.random() * (maxForThisOption + 1)
        );
    ...
    if (currentSum !== 100 && optionsForPoll.length > 0) {
      const optionToAdjust =
        optionsForPoll.find(
          (opt) => opt.text === correctAnswerText && isCorrectAnswerDisplayed
        ) || optionsForPoll[0]; // Prefer adjusting correct if possible
      if (optionToAdjust && results[optionToAdjust.text] !== undefined) {
        results[optionToAdjust.text] = Math.max(
          0,
          Math.min(100, results[optionToAdjust.text] + (100 - currentSum))
        );
      }
    }
    ...
    if (currentSum !== 100 && optionsForPoll.length > 0) {
      ...
      if (results[firstKey] !== undefined) {
        results[firstKey] = Math.max(
          0,
          Math.min(100, results[firstKey] + (100 - currentSum))
        );
      }
    }
    ...
    toast({
      ...
      description: "The audience has cast their votes!",
    });
  }, [currentQuestion, isAudiencePollUsed, displayedAnswers, toast]);

  const saveScore = useCallback(
    async (name: string, userId: string, timeTakenMs: number | null) => {
      if (!firestore) {
      ...
      const actualTimeTaken =
        typeof timeTakenMs === "number" && timeTakenMs > 0 ? timeTakenMs : 0;
      console.log(
        `[saveScore] Attempting to save. User: ${name}, ID: ${userId}, Score: ${score}, TimeTakenMs: ${actualTimeTaken}`
      );
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          console.log(
            `[saveScore] Existing score for user ${userId}: Score ${existingData.score}, Time ${existingData.timeTakenMs}ms`
          );
          if (score > existingData.score) {
            ...
            (actualTimeTaken < existingData.timeTakenMs ||
              typeof existingData.timeTakenMs !== "number")
          ) {
            shouldWrite = true;
            message = `Score of ${score.toLocaleString()} matches previous, but with a faster time! (${(
              actualTimeTaken / 1000
            ).toFixed(2)}s vs ${
              existingData.timeTakenMs
                ? (existingData.timeTakenMs / 1000).toFixed(2) + "s"
                : "N/A"
            }).`;
          } else {
            message = `Your score of ${score.toLocaleString()} (time: ${(
              actualTimeTaken / 1000
            ).toFixed(
              2
            )}s) did not beat your previous high score of ${existingData.score.toLocaleString()} (time: ${
              existingData.timeTakenMs
                ? (existingData.timeTakenMs / 1000).toFixed(2) + "s"
                : "N/A"
            }).`;
          ...
          shouldWrite = true;
          message = `First score of ${score.toLocaleString()} (time: ${(
            actualTimeTaken / 1000
          ).toFixed(2)}s) saved!`;
          console.log(
            `[saveScore] No existing score found for user ${userId}. Saving new score.`
          );
        }

        if (shouldWrite) {
          const scoreData: Omit<
            FirestoreScoreEntry,
            "id" | "date" | "timestampMillis"
          > & { timestamp: any } = {
            name: name,
            score: score,
            userId: userId,
          ...
          console.log(
            `[saveScore] Firestore write successful for user ${userId}.`
          );
          toast({
            title: "Score Saved!",
            description: message,
          ...
          console.log(
            `[saveScore] Score not written for user ${userId} as it was not better.`
          );
          toast({
            title: "Score Status",
            description: message,
        ...
        toast({
          title: "Error Saving Score",
          description: `Could not save your score. Error: ${
  ...
  return {
    ...
    totalQuestions:
      questions.length > 0 ? questions.length : TOTAL_QUESTIONS_IN_GAME,
    timeTakenMs:
      gameEndTime && gameStartTime ? gameEndTime - gameStartTime : null,
