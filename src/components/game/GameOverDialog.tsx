...
export function GameOverDialog({
  ...
  const descriptionText = isWinner
    ? `Incredible! You've conquered all questions in ${gameName}! Your final score is an amazing ${score.toLocaleString()} points. Time: ${formatTime(
        timeTakenMs
      )}.`
    : `Congratulations${
        user?.displayName ? `, ${user.displayName}` : ""
      }! You scored ${score.toLocaleString()} points. Time: ${formatTime(
        timeTakenMs
      )}. ${
        score > 0
    ...
    >
      <DialogContent
        className={cn(
          ...
          isWinner &&
            "sm:max-w-lg border-none shadow-2xl shadow-yellow-500/70 bg-gradient-to-br from-purple-600 via-pink-500 to-yellow-400 text-white"
        )}
      >
        <DialogHeader className="text-center pt-6 sm:pt-8 px-4 sm:px-6">
          <div className="flex justify-center items-center space-x-2 mb-4">
            {isWinner ? (
              <>
                <Sparkles
                  className="h-7 w-7 sm:h-8 sm:w-8 text-yellow-300 animate-ping opacity-75"
                  style={{ animationDelay: "0s" }}
                />
                <Crown
                  className="h-10 w-10 sm:h-12 sm:w-12 text-yellow-400 animate-bounce"
                  style={{ animationDelay: "0.1s" }}
                />
                <Trophy className="h-12 w-12 sm:h-14 sm:w-14 text-yellow-500 animate-pulse" />
                <PartyPopper
                  className="h-10 w-10 sm:h-12 sm:w-12 text-pink-400 animate-bounce"
          ...
          <DialogTitle
            className={cn(
              ...
              isWinner
                ? "text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-white to-pink-300 drop-shadow-lg text-3xl sm:text-4xl md:text-5xl py-2"
                : "text-primary"
            )}
          >
            {titleText}
        ...
        <DialogFooter className="flex-col sm:flex-row sm:justify-between space-y-3 sm:space-y-0 sm:space-x-2 pt-6 pb-6 sm:pb-8 px-6 sm:px-8">
          <Button
            ...
            className={cn(
              ...
              isWinner &&
                "border-yellow-200/70 text-yellow-100 hover:bg-white/20 hover:text-white font-semibold"
            )}
            disabled={isSaving}
          >
          ...
          {(score > 0 || isWinner) && user && (
            <Button
              ...
              className={cn(
                ...
                isWinner &&
                  "bg-gradient-to-r from-yellow-500 via-pink-400 to-purple-500 hover:opacity-95 text-white shadow-lg hover:shadow-xl transform hover:scale-105 font-semibold"
              )}
            >
              {isSaving ? (
