...
const GenerateTriviaQuestionsInputSchema = z.object({
  ...
  difficulty: z
    ...
    .optional()
    .describe(
      'Optional: The desired difficulty for all questions (e.g., "Easy", "Medium", "Hard"). If "Mixed" or not provided, a range of difficulties will be generated.'
    ),
  category: z
    .string()
    .optional()
    .describe(
      'Optional: The desired category for all questions (e.g., "Sports", "History"). If "General Knowledge" or not provided, questions will cover various topics.'
    ),
  requestIdentifier: z
    .string()
    .optional()
    .describe(
      "An optional unique identifier for the game session to help ensure question variety across multiple plays."
    ),
  variationHint: z
    .string()
    .optional()
    .describe(
      "An optional random string to further encourage unique question sets for each request."
    ),
});
...
const GeneratedQuestionSchema = z.object({
  id: z
    .string()
    .describe(
      "A unique identifier for the question (e.g., generated UUID or sequential, must be unique within THIS set of questions)."
    ),
  text: z.string().min(1).describe("The text of the trivia question."),
  answers: z
    .array(AnswerSchema)
    .length(4)
    .describe("An array of exactly four answer options."),
...
const AIGeneratedTriviaOutputSchema = z.object({
  questions: z
    ...
    .describe(
      "An array of generated trivia questions, matching the GeneratedQuestionSchema structure."
    ),
});

export type AIGeneratedTriviaData = z.infer<
  typeof AIGeneratedTriviaOutputSchema
>;

export async function generateTriviaQuestions(
  input: GenerateTriviaQuestionsInput
): Promise<{ questions: Omit<Question, "points" | "imageUrl">[] }> {
...
const generateTriviaQuestionsFlow = ai.defineFlow(
  {
    ...
    outputSchema: z.object({ questions: z.array(GeneratedQuestionSchema) }),
  },
  async (input) => {
    let promptResult;
    ...
    try {
      ...
      if (!rawAiOutputFromPrompt) {
        let errorDetails =
          "AI prompt returned no structured output. The AI response might be malformed, empty, or failed Zod parsing internally by Genkit.";
        if (
          promptResult &&
          (promptResult as any).candidates &&
          (promptResult as any).candidates[0]?.finishReason
        ) {
          errorDetails += ` Finish Reason: ${
            (promptResult as any).candidates[0].finishReason
          }.`;
          if ((promptResult as any).candidates[0].finishMessage) {
            errorDetails += ` Finish Message: ${
              (promptResult as any).candidates[0].finishMessage
            }.`;
          }
        }
        console.error(
          "[generateTriviaQuestionsFlow] AI prompt returned no structured output (output is null or undefined). Input:",
          JSON.stringify(input, null, 2),
          "Full Genkit Result (if available):",
      ...
      console.error(
        "[generateTriviaQuestionsFlow] Critical error during AI prompt execution or initial parsing. Input:",
        JSON.stringify(input, null, 2),
        "Error:",
        e
      );
      ...
      let errorMessage =
        "AI prompt execution failed or returned malformed data.";
      if (e instanceof z.ZodError) {
        // Corrected ZodError check
        errorMessage = `AI output failed Zod validation: ${e.errors
          .map((err) => `${err.path.join(".")} - ${err.message}`)
          .join(", ")}`;
        console.error(
          "[generateTriviaQuestionsFlow] Zod validation issues (details):",
    ...
    if (
      !rawAiOutputFromPrompt.questions ||
      !Array.isArray(rawAiOutputFromPrompt.questions)
    ) {
      console.error(
        '[generateTriviaQuestionsFlow] AI output for "questions" is not an array, even after initial Genkit parsing. Input:',
        JSON.stringify(input, null, 2),
        "Received Output:",
        JSON.stringify(rawAiOutputFromPrompt, null, 2)
      );
      throw new Error(
        'AI returned malformed data: "questions" field was not an array.'
      );
    }

    if (
      rawAiOutputFromPrompt.questions.length === 0 &&
      input.numberOfQuestions > 0
    ) {
      console.warn(
        "[generateTriviaQuestionsFlow] AI returned an empty list of questions when questions were requested. Input:",
        JSON.stringify(input, null, 2)
      );
    }
    ...
    for (const [index, qSource] of rawAiOutputFromPrompt.questions.entries()) {
      ...
      try {
        ...
        if (
          ...
          console.warn(
            `[Validation] ${currentQuestionForLog} has missing/invalid ID "${qSource.id}". Assigning fallback.`
          );
          questionId = `gen_q_fallback_${index}_${Date.now()}_${Math.random()
            .toString(36)
            .substring(2, 7)}`;
        } else if (questionIdsInSet.has(questionId)) {
          console.warn(
            `[Validation] ${currentQuestionForLog} has duplicate ID "${questionId}" within this set. Assigning fallback.`
          );
          questionId = `gen_q_dup_fallback_${index}_${Date.now()}_${Math.random()
            .toString(36)
            .substring(2, 7)}`;
        }
        ...
        if (
          !qSource.text ||
          typeof qSource.text !== "string" ||
          qSource.text.trim() === ""
        ) {
          throw new Error(
            `${currentQuestionForLog} has missing or empty text.`
          );
        }

        if (
          !qSource.answers ||
          !Array.isArray(qSource.answers) ||
          qSource.answers.length !== 4
        ) {
          throw new Error(
            `${currentQuestionForLog} (Text: "${qSource.text.substring(
              0,
              30
            )}...") must have exactly four answers, found ${
              qSource.answers?.length || 0
            }.`
          );
        }

        const correctAnswers = qSource.answers.filter(
          (a) => a.isCorrect === true
        ).length;
        if (correctAnswers !== 1) {
          throw new Error(
            `${currentQuestionForLog} (Text: "${qSource.text.substring(
              0,
              30
            )}...") must have exactly one correct answer, found ${correctAnswers}.`
          );
        }
        ...
        for (const [ansIdx, ans] of qSource.answers.entries()) {
          if (
            ...
            throw new Error(
              `Answer option ${
                ansIdx + 1
              } for ${currentQuestionForLog} (Text: "${qSource.text.substring(
                0,
                30
              )}...") is missing text.`
            );
          ...
          if (typeof ans.isCorrect !== "boolean") {
            throw new Error(
              `Answer option ${
                ...
                0,
                30
              )}...") has an invalid 'isCorrect' field (must be boolean).`
            );
          }
        }
        ...
        if (!DifficultyEnum.safeParse(questionDifficulty).success) {
          console.warn(
            `[Validation] ${currentQuestionForLog} (Text: "${qSource.text.substring(
              0,
              30
            )}...") has invalid/missing difficulty "${
              qSource.difficulty
        ...
        const safeQSource = JSON.stringify(
          qSource,
          (key, value) =>
        ...
        console.warn(
          `[generateTriviaQuestionsFlow] Skipping question at index ${index} due to validation error: ${validationError.message}. Original data (truncated):`,
          safeQSource
        );
      }
    }
    ...
    if (validatedQuestions.length === 0 && input.numberOfQuestions > 0) {
      console.error(
        "[generateTriviaQuestionsFlow] No questions passed validation, or AI returned no usable questions initially. Input:",
        JSON.stringify(input, null, 2)
      );
      throw new Error(
        "AI-generated questions failed validation, or no questions were returned by the AI that could be used."
      );
    }

    if (validatedQuestions.length < input.numberOfQuestions) {
      console.warn(
        `[generateTriviaQuestionsFlow] Returning ${validatedQuestions.length} valid questions, but ${input.numberOfQuestions} were requested. Some AI-generated questions may have failed validation. Input:`,
        JSON.stringify(input, null, 2)
      );
    }
